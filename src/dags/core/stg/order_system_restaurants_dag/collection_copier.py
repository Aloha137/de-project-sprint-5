...
class CollectionCopier:
    ...
    def __init__(self, collection_loader: CollectionLoader, pg_dest: PgConnect, pg_saver: PgSaver, logger: Logger) -> None:
    ...
    def run_copy(self, collection: str) -> int:
        WF_KEY = f"example_ordersystem_{collection}_origin_to_stg_workflow"
        ...
        with self.pg_dest.connection() as conn:
            ...
            load_queue = self.collection_loader.get_documents(collection, last_loaded_ts, self._SESSION_LIMIT)
            self.log.info(f"Found {len(load_queue)} documents to sync from {collection} collection.")
            if not load_queue:
            ...
            i = 0
            for d in load_queue:
                self.pg_saver.save_object(collection, conn, str(d["_id"]), d["update_ts"], d)

                i += 1
                if i % self._LOG_THRESHOLD == 0:
                    self.log.info(f"processed {i} documents of {len(load_queue)} while syncing documents.")
