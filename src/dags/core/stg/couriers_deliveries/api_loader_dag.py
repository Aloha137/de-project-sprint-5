...
from core.stg.couriers_deliveries.api_loader import APIClient, APIDataLoader
...
def sprint5_project_api_loader():
    ...
    headers = {'X-Nickname': 'alalmelnik', 'X-Cohort': '11', 'X-API-KEY': '25c27781-8fde-4b30-a22e-524044a7580f'}
    
    @task()
    def load_couriers():
        params_couriers = {'sort_field': 'id','sort_direction': 'asc', 'limit' : 50, 'offset' : 0}
        apiclient = APIClient(base_url, dwh_pg_connect, 'couriers', headers)
        load_queue = apiclient.get('couriers', params_couriers  )
        print('Данные для загрузки: ', load_queue)
        dataLoader = APIDataLoader(apiclient, dwh_pg_connect, 'couriers',log )
    ...
    @task()
    def load_deliveries():
        apiclient = APIClient(base_url, dwh_pg_connect, 'deliveries', headers)
        ...
        print('Данные для загрузки: ', load_queue)
        dataLoader = APIDataLoader(apiclient, dwh_pg_connect, 'deliveries',log)
        if len(load_queue) != 0: 
            for d in load_queue:
                for c in d:
                    print(c)    
