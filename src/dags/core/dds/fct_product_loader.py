...
from core.dds.bonus_event_repository import BonusEventRepository
from core.dds.dds_settings_repository import StgEtlSettingsRepository, EtlSetting
from core.dds.orders_repositories import OrderDdsRepository
from core.dds.products_loader import ProductDdsObj, ProductDdsRepository
...
class FctProductsLoader:
    ...
    def load_product_facts(self):
        with self.dwh.connection() as conn:
            ...
            print("Список продуктов обработан: ", prod_dict)

            proc_cnt = 0
            for payment_raw in load_queue:
                print("Одна из строк таблицы events: ", payment_raw)
                payment_obj = BonusPaymentJsonObj(json.loads(payment_raw.event_value))
                print("Привели эту строку в другой объект: ", payment_obj)
                order = self.dds_orders.get_order(conn, payment_obj.order_id)
                print("Получили следующий заказ из уже имеющихся в dds: ", order)
                if not order:
                ...
                print("Факты для загрузки: ", facts_to_load)
